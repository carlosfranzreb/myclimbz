...
function count_climbs(data) {
	let out = new Map();
	for (let [key, value] of data) out.set(key, value.length);
	return out;
}
...
function count_climbs_sent(data) {
	...
	for (let [key, value] of data) {
		...
		for (let climb of value) {
			if (climb.sent === true) out.set(key, out.get(key) + 1);
		}
	}
	return out;
}
...
function success_rate(data) {
	...
	for (let [key, value] of data) {
		...
		if (value.length == 0) out.set(key, 0);
		else out.set(key, sent_climbs.length / value.length);
	}
...
function avg_grade(data) {
	out = new Map();
	for (let [key, value] of data) out.set(key, get_avg_level(value));
	return out;
}
...
function max_grade(data) {
	out = new Map();
	for (let [key, value] of data) out.set(key, get_max_level(value));
	return out;
}
