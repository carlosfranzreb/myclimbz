...
function show_plot() {
	...
	if (x_axis.includes("level")) {
		...
		if (date_len == 4) {
			// Sort by year
			out = new Map(
				Array.from(out).sort((a, b) => parseInt(a[0]) - parseInt(b[0]))
			);
		} else if (date_len == 7) {
		...
		out = new Map(
			...
			Array.from(out).sort((a, b) => parseFloat(a[0]) - parseFloat(b[0]))
		);
		...
		for (let key = first_key; key <= last_key; key += step) {
			if (out.has(key)) new_out.set(key, out.get(key));
			else new_out.set(key, 0);
		}
		out = new_out;
	} else {
		// Sort the map by the keys
	...
	let x = d3.scaleBand().range([0, WIDTH]).domain(out.keys()).padding(0.2);
	...
	if (y_axis_options[y_axis]["axis_labels"] != null) {
		...
		let y_axis_obj = d3.axisLeft(y).tickFormat(function (d) {
			return y_labels.get(d);
		});
		svg.append("g").call(y_axis_obj);
	} else svg.append("g").call(d3.axisLeft(y));
