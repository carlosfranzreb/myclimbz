...
(function (root, factory) {
  ...
  function (plugin) {
      ...
      if (format) {
        switch (format) {
          case "ISO_8601":
            ...
            break;
          case "RFC_2822":
            date = moment(content, "ddd, MM MMM YYYY HH:mm:ss ZZ").format(
              "YYYYMMDD"
            );
            break;
          case "MYSQL":
    ...
    Columns.prototype.sort = function (column, direction, init) {
      ...
      let get_entries = function (arr) {
        each(rows, function (tr) {
          ...
          let content = cell.hasAttribute("data-content")
      ...
      if (classList.contains(th, "asc")) {
        ...
        if (
          alpha.length > 0 &&
          alpha[0].value.match(/^\d{1,2}\/\d{1,2}\/\d{4}$/)
        ) {
          alpha.sort((a, b) => {
          ...
          if (entries.length > 0 && entries[0].value.startsWith("V")) {
            entries.sort((a, b) => {
              return (
                hueco_grades_list.indexOf(a.value) -
                hueco_grades_list.indexOf(b.value)
              );
            });
          } else {
            entries.sort((a, b) => {
              return (
                font_grades_list.indexOf(a.value) -
                font_grades_list.indexOf(b.value)
              );
            });
          }
        } else {
    ...
    proto.init = function (options) {
      if (
        this.initialized ||
        classList.contains(this.table, "dataTable-table")
      ) {
        return false;
      }
              ...
              if (that.selectedColumns.indexOf(i) > -1) {
                each(that.columnRenderers, function (o) {
                  if (o.columns.indexOf(i) > -1) {
                    cell.innerHTML = o.renderer.call(
                      that,
                      cell.data,
                      cell,
                      row
