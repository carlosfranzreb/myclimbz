...
function check_option_validity(name, selected_index, select) {
    ...
    if (name === "Grade" || name === "Grade felt") {
        ...
        for (let row of csv_data) {
            if (row[selected_option] !== "" && !font_grades_regex.test(row[selected_option]) && !hueco_grades_regex.test(row[selected_option])){
                alert("The column " + selected_option + " contains invalid grades.");
                return false;
            }
        }
    }
    else if (name === "No. of attempts" || name === "No. of attempts to send" || name === "No. of sessions" || name === "Height" || name === "Inclination") {
        //check that all the values in the column of csv_data are numbers
        for (let row of csv_data) {
            if (isNaN(row[selected_option])){
                alert("The column " + selected_option + " contains non-numeric values.");
                return false;
            }
        }
    }
        ...
        for (let row of csv_data) {
            if (isNaN(row[selected_option]) || row[selected_option] < 1 || row[selected_option] > 10){
                alert("The column " + selected_option + " contains values outside the range 1-10.");
                return false;
            }
        }
    }
        ...
        for (let row of csv_data) {
            if (row[selected_option] !== "yes" && row[selected_option] !== "true" && row[selected_option] !== "no" && row[selected_option] !== "false" && row[selected_option] !== ""){
                alert("For the Sent column only the values 'yes', 'true', 'no', 'false' or empty are allowed.");
                return false;
            }
        }  
    }
        ...
        for (let row of csv_data) {
            if (row[selected_option] !== "" && parseTime(row[selected_option]) === null){
                alert("The column " + selected_option + " contains invalid dates. The dates should be in the format 'dd/mm/yyyy' or empty.");
                return false;
            }
        }
    }
    ...
    for(let key in CSV_IMPORT_OPTIONS){
        ...
        select.addEventListener("change", function() {
            ...
            if (select.selectedIndex !== 0 && csv_data !== null) {
                ...
                let select_label = document.querySelector('label[for="' + select.id + '"]').textContent;
                is_valid = check_option_validity(select_label, select.selectedIndex, select);
            }
            if (is_valid) {
                disableSelectedOption(select.selectedIndex, selected_indices[select.id], select.id);
            }
            else {
...
//TODO delete things when window closed/reloaded etc
...
document.getElementById("confirm_csv_import_button").addEventListener("click", function (e) {
    ...
    for (let key in CSV_IMPORT_OPTIONS) {
        ...
        if (selected_index !== 0) {
            unused_cols = unused_cols.filter(col => col !== select.options[selected_index].value);
            let selected_option = select.options[selected_index].value;
            csv_data.forEach(row => {
                row[key] = row[selected_option];
                delete row[selected_option];
            });
        }
    }
